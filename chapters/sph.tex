...
The map $\Phi \mapsto \sphc{\Phi}$ is called \emph{continuous interpolation}. Certainly, the interpolation kernel $w_{h}$ cannot be arbitrary. We aim to approximate the function $\Phi$ by its continuous interpolation $\sphc{\Phi}$, that is
...
To obtain a good approximation, the kernel must satisfy \autocite[p.~60]{liuSmoothedParticleHydrodynamics2004}
...
It is evident that for the Wendland kernel \Cref{eq:WENDLAND-KERNEL}, $w_h\left(\frac{\vb{r}}{h}\right)$ for $\abs{\vb{r}} \leq 2 h$, meaning $4 h = \diam \supp w_h$.
