...
In section \Cref{sec:LIMITATION} we have discussed how, in the current stage of development, the standard SILVA possess serious limitations for simulating compressible flows. In \Cref{sec:SILVA_FAILURE} we showed that the exact hydrostatic balance is not preserved and the total hydrostatic residual $\abs{\grad p/ \rho + \vb{g}}$ is dominated by the boundary residual. Moreover, we have observed that while in the domain's interior the preservation can be improved by refining the mesh (but still \emph{not completely resolved}) the boundary hydrostatic residual seems unaffected by the refinement.
