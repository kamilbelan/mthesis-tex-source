...
\begin{frame}{The failure of standard SPH: velocity field}
	\solofigure{figures/results/field_SPH.png}{Nonzero velocity field computed by standard SPH - a failure of hydrostatic balance.}{fig:SPH_FIELD}
\end{frame}

\begin{frame}{The success of well-balanced $p-\theta$: velocity norms}
	\solofigure{figures/results/stationary_norms_compare.pdf}{Comparison of standard SPH and well-balanced $p-\theta$. $p-\theta$ preserves hydrostatic balance exactly.}{fig:WB_NORMS}
\end{frame}
...
\begin{frame}{Tensile instability: clumping}
...
\begin{frame}{Summary}
	\begin{columns}[c,onlytextwidth]
		...
		\begin{itemize}
			...
			\item large scale hydrostatic balance is problematic for standard SPH
...
\begin{qaframe}{Oponent, otázka 1}
	...
	\begin{answer}
		\centering
		% No float here: inside the answer box the "Figure N:" caption and the float
		% skips cost about 30pt, which is what pushed these two frames ~48pt past the
		% body.  0.30\textheight is what is left once the question box, the answer
		% label and the credit line have taken their share (0.38 still ran 15pt over).
		\includegraphics[height=0.30\textheight,keepaspectratio]{figures/reference.jpg}\\[0.3em]
		{\tiny\itshape Reference results. \href{https://doi.org/10.1175/MWR-D-10-05042.1}{Monthly Weather Review 139, 9; 10.1175/MWR-D-10-05042.1}}
	\end{answer}
\end{qaframe}
...
\begin{qaframe}{Vedoucí, otázka 2}
	...
	\begin{answer}
		\textbf{Obecná evoluce}
		\begin{equations}[single,!,numberline=all]
		\label{eq:OBECNA}
		\dv{F}{t} = \partial_t F + \{F, E\},
		\end{equations}

		\textbf{SPH závorka}
		\begin{equations}[single,!,numberline=all,noskip]
		\label{eq:PB}
			\{F\left(\vb{x}_a, \vb{M}_a\right), G\left(\vb{x}_a, \vb{M}_a\right)\} = \sum_{a}\left(\pdv{F}{x^i_a} \pdv{G}{M_{a,i}} - \pdv{F}{M_{a,i}}\pdv{G}{x^i_a}\right),
		\end{equations}

		\textbf{SPH energie}
		\begin{equations}[single,!,numberline=all]
		\label{eq:ENERGY}
		E = \sum_{a} \left(\frac{\abs{\vb{M}_{a}}^{2}}{2 m_a} + \frac{m_a}{\rho_a}e\left(\link[h]{\rho_a}, \eta_a\right)\right),
		\end{equations}
...
\begin{qaframe}{Vedoucí, otázka 2}
	\begin{answer}[Odpověď (pokračování)]
		\textbf{Evoluce rychlosti}
		\begin{equations}[single,!,numberline=all]
		\label{eq:CANONICAL}
		\dv{\vb{v}_a}{t} = \frac{1}{m_a} \dv{\vb{M}_a}{t} = - \frac{1}{m_a} \link[h]{\grad_a} E,
		\end{equations}
		...
		\begin{equations}[single,!,numberline=all]
		\label{eq:DENSITY}
		\link[h]{\rho_a} = \sum_{b} m_b w_{ab}\left(h_b\right)
		\end{equations}
	\end{answer}
\end{qaframe}
